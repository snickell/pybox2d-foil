...
#                    ("Friction Forces", "drawFrictionForces"),  #unused
    ...
    def __init__(self,settings, **params):
        ...
        # Hertz slider
        self.tr()
        e = gui.HSlider(settings.hz,60,1000,size=20,width=100,height=16,name='hz')
    ...
    * Handles all user input.
    ...
    def __init__(self):
        ...
        try:
            ...
            print "NOT using psycho to speed up python"
            pass
    ...
    def run(self):
        ...
        while running:
            ...
            self.settings.draw = False
            # Run the simulation loop
            for i in range(int(self.settings.numUndrawnFrames)):
                self.SimulationLoop()
            ...
            pygame.display.flip()
            clock.tick_busy_loop(self.settings.hz / (self.settings.numUndrawnFrames + 1.0))
    ...
    def Step(self, settings):
        ...
        if settings.drawFPS: #python version only
            realtime_scalar = float(self.fps * (1.0 + settings.numUndrawnFrames)) / settings.hz
            self.DrawString(5, 200, "FPS %d, %3.2fx realtime" % (self.fps, realtime_scalar))
            self.textLine += 15
    ...
    def SimulationLoop(self):
        ...
        if self.settings.draw: self.SetTextLine(30)
        if self.settings.draw: self.DrawString(5, 15, self.name)
